...
第 2 章：分析题目要求，给出大致设计方向。

第 3 章：热力计算与流道基本尺寸的确定。
...
4. 喷嘴叶片外径 $$ D_0 \approx 2 \sqrt {R_N^2 + (ab)^2 + 2 (ab) \sin {\alpha_{1A}} R_N} = mm $$
...
13. 以轴线为中心的等直径圆柱面上导流段的曲线可按二次抛物线方程 $ y = \frac {x^2} {2p} $ 确定，而
    ...
    $$ \tan {\beta_2} = \frac {c_{2r}} {u_2 - c_{2u}} = \frac {c_2} {u_2} = 0.59060 $$
...
首先草图绘制中间的轴和轮背的轮廓，旋转 $ 360 ^\circ $ 构建成基体，然后草图绘制工作段，最后在出口导流段新建几个参考面，调整好每个参考面距离轴心的距离，在每个参考面中绘制出口导流段的形状，然后通过“通过曲线截面”新建一个没有厚度的截面，最后通过加厚操作构建出一个叶片，然后用阵列旋转复制一圈即可。

![叶轮主体段](/images/2019-04-12-graduation-project/impeller-1.png)

![叶轮整体](/images/2019-04-12-graduation-project/impeller-2.png)

## 实验平台搭建

通过上述设计之后，我们可以进行实验平台搭建了，整套系统的流程图如下。

压缩机 -> 换热器 -> 透平膨胀机进口 -> 透平膨胀机（产生冷量）-> 低温室 -> 换热器（回热，交换冷量，避免浪费） -> 排空。

其中换热器、透平膨胀、低温室在冷箱内部，压缩机、制动风机在低温是外部。实际应用中，低温室可以通过保温管道引出制冷机的冷箱，使用另一个冷箱。膨胀机在冷箱内部，其主轴直接与冷箱外部的制动风机相连，这部分没有密封，通过气体轴承间接密封。

### 压缩机系统

压缩机系统使用 0.8 MPa 带水冷却器、油水分离器、过滤干燥器的螺杆式压缩机，通过进口阀门调节作用来控制阀后压力可以在 0.2-0.7 MPa 调节，实验室则按照给定要求压力调节。通过微调压缩机系统来控制转速。

### 气体轴承膨胀机

本实验设计转速约 75,000 r/min，径向使用气体轴承支撑，

### 板翅式换热器

想要制冷的话，膨胀机、低温室中的气体必须流动起来，这样才能带动叶轮，但是这样势必会导致一部分气体带走一部分冷量。如果不采用回热的话，膨胀机进口的温度永远都是压缩气体的原始温度（近似环境温度），膨胀机出口的温度也会固定在仅有一次膨胀的出口温度。气体工质是开式循环（即不循环，出口不会与进口有关联），但是不能让冷量也是“开式循环”，可以通过回热器可以使冷量部分保存下来。

板翅式换热器的效率会影响降温效率，理想情况下等效于低温室出口低压气体直接变成高压气体再流入膨胀机。换热效率还会影响最低温度，当整套系统达到最低温度时，系统参数达到平衡，这时如果想再降低温度的话，可以尝试换用更高效的换热器，相当于变相的降低膨胀机进口温度，必然导致膨胀机出口温度下降。板翅式换热器只是影响最低温度和降温曲线的一个原因，通常应该主要从膨胀机设计方向考虑。

本套实验装置采用铝制钎焊板翅式换热器。

### 测量系统

实验冷箱内部大量采用热电偶和压力传感器，除此之外来自压缩机的气体管道的阀门后有一台机械式的压力表，另外有一个光电式传感器负责测量主轴转速。测量系统全部通过导线连接到数据采集器，并保存成可读取格式供后续分析处理。实验中使用铜-康铜热电偶，测温范围 -200 ℃ - 400℃，测量精度为 0.1K，以标态冰水混合物为基准点。

低温室的每一段管均有热电偶测量温度。回热器进口（装置进口），回热器出口（工作轮进口），工作轮出口（低温室进口），低温室出口（回热器进口），回热器出口（排空口）。工作轮进出口和回热器进出口还应布置 4 个测压点。如果采集设备有足够的接口，可以分别在回热器和工作轮的端点取两次温度和压力，这样可以分析并部分了解低温下气体在管道中的流动状态。尤其是液空出现之后，管道中的液空流动状态。

### 冷箱

冷箱采用高真空多层绝热，内外夹层包裹防辐射屏，并抽真空，减少跑冷损失。
...
本文完成了给定的设计要求，利用已有一维设计理论，给出一套透平膨胀机的设计参数。并且给出了 UG 建模的结果，利用这个建模结果可以进一步进行数值模拟。最后给出了一套实验测试方案，使用这一套测试方案可以测出膨胀机中各个点的物性参数，进一步研究可以计算出真实效率。

### 展望

由于本文完全是凭借理论制造，作者才疏学浅，见识浅薄，没有亲眼见到过膨胀机运行，对膨胀机参数随运行状态变化没有大致了解，对膨胀机的各个位置的效率了解仅限于书本，不能保证设计出的参数可以真实可用。

下一步的工作可以实际造出一两个叶轮，实际运行一下，测试一下各个参数随给定参数的变化，然后在结合所学理论知识分析一下其中的原理，即使实验精度不高，但是数据变化的趋势也是有参考价值的。

对于目前而言，没法做实验的情况下，无法确定数据的正确性，我也是第一次接触这方面知识。与课堂所学知识不同，课堂的知识是孤立的，我们通常仅研究某一个公式中的自变量。实际设计过程由于影响因素过多，多个公式级联，无法直观地感受到设计的正确与否。

虽然现在有高精度的数值模拟，但是在没有实验验证的前提下，数值计算的可信度并不高。数值计算中需要考虑主要因素，忽略其余部分次要因素，但实际情况下，可能在高速或者低温等非常规工况下，次要因素上升为主要因素。数值计算有至少一组解与已知实验相吻合，那么数值计算将具有说服力。自然科学是一门实验性科学，脱离了实际物质变化规律的理论是没有意义的。
